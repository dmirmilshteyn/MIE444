...
void determineStallPWM() {
  if (determineStallPWMDone == 0) {
    ...
    do {
      ...
      i++;
    } while (leftMotorCount < GEAR_RATIO * ENCODER_TEETH_COUNT / 10 && rightMotorCount < GEAR_RATIO * ENCODER_TEETH_COUNT / 10); //((previousLeftMotorCount - leftMotorCount) / (1 / 1000) < (1204 * 0.05));
    stallPWM = i;
  }
  leftMotorCount = 0;
  rightMotorCount = 0;
